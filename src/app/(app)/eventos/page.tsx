...
export default function EventsPage() {
  ...
  const [typeFilter, setTypeFilter] =
    useState<EventType | "Todos">("Todos");

  const [statusFilter, setStatusFilter] =
    useState<EventStatus | "Todos">("Todos");

  const [isFormOpen, setIsFormOpen] = useState(false);

  const [eventToEdit, setEventToEdit] =
    useState<ChoirEvent | null>(null);
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((choirEvent) => {
      const title = choirEvent.title.toLocaleLowerCase("es-MX");

      const location =
        choirEvent.location?.toLocaleLowerCase("es-MX") ?? "";

      const description =
        choirEvent.description?.toLocaleLowerCase("es-MX") ?? "";
      ...
      const matchesType =
        typeFilter === "Todos" ||
        choirEvent.event_type === typeFilter;

      const matchesStatus =
        statusFilter === "Todos" ||
        choirEvent.status === statusFilter;
  ...
  function openEditForm(choirEvent: ChoirEvent) {
    setEventToEdit(choirEvent);
    setIsFormOpen(true);
  }
  ...
  async function handleFormSubmit(
    eventPayload: ChoirEventPayload
  ) {
    try {
      setErrorMessage("");
  ...
  async function handleDelete(choirEvent: ChoirEvent) {
    ...
    try {
      ...
      await deleteEvent(choirEvent.id);

      setEvents((currentEvents) =>
        currentEvents.filter(
          (currentEvent) => currentEvent.id !== choirEvent.id
        )
      );
    } catch (error) {
      const message =
          ...
          {isLoading ? (
            ...
            <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Registra el primer evento o modifica los filtros de
                búsqueda.
              </p>
            </div>
          ) : (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {filteredEvents.map((choirEvent) => (
                <EventCard
                  key={choirEvent.id}
                  event={choirEvent}
                  onEdit={openEditForm}
...
interface EventCardProps {
  event: ChoirEvent;
  onEdit: (event: ChoirEvent) => void;
  onDelete: (event: ChoirEvent) => void;
}
    ...
    <article className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="mt-5 flex-1">
        <p className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
          {getEventCountdown(event.event_date)}
        </p>
      </div>
